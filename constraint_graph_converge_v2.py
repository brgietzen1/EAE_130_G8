...
def takeoff_constraint_converge(s_TO, rho_ground, rho_sea, cl_max_TO, eta_p):
    ...
    for i in range(len(S)):
        ...
        while converged == False:
            results = we.solve_takeoff_weight_2(S_0, P[i], S_design = 12214.12 / 19.7, P_design = 12214.12 / 17.8)
...
def climb_constraint_converge(e, AR, k_s, cd_0, cl_max_climb, rho, eta_p, G, type):
    S = np.linspace(200, 600, 1000)
    P = np.zeros_like(S)
    for i in range(len(S)):
        S_0 = S[i]
...
def stall_constraint_converge(v_stall, rho_cruise, cl_max):
    ...
    for i in range(len(P)):
        ...
        converged = False
        while converged == False:
            results = we.solve_takeoff_weight_2(S[i], P_0, S_design = 12214.12 / 19.7, P_design = 12214.12 / 17.8)
            takeoff_weight, empty_weight_frac, fuel_weight_frac,\
            empty_weight, fuel_weight, iterations = results
            W = takeoff_weight
...
S_takeoff, P_takeoff, = takeoff_constraint_converge(s_TO=1000, rho_ground=.002378, rho_sea=.002378, cl_max_TO=1.7333, eta_p=0.9)
S_climb1, P_climb1 = climb_constraint_converge(e=1.03, AR=7.5, k_s=1.3, cd_0=0.0436, cl_max_climb=1.7333, rho=.002378, eta_p=0.9, G=0.04, type='takeoff')
S_climb2, P_climb2 = climb_constraint_converge(e=0.97, AR=7.5, k_s=1.3, cd_0=0.1136, cl_max_climb=2.0, rho=.002378, eta_p=0.9, G=0.03, type='balked')
S_cruise, P_cruise = cruise_constraint_converge(e=1.10, AR=7.5, v_cruise=212, eta_p=0.9, q_cruise=(1/2*.002048 * 212**2), cd_0=0.02862)
S_ceiling, P_ceiling = ceiling_constraint_converge(e=1.10, AR=7.5, cd_0=0.02862, rho_cruise=.002048, cl_max_cruise=1.6, eta_p=0.9)
S_maneuver, P_maneuver = maneuver_constraint_converge(e=1.10, AR=7.5, v_stall_flight=145, R_turn=1800, q_cruise=(1/2*.002048 * 212**2), cd_0=0.02862, rho_ground=.002378, cl_max=1.6, eta_p=0.9)
S_stall, P_stall = stall_constraint_converge(v_stall=145, rho_cruise=0.002048, cl_max=1.6)
S_landing, P_landing = landing_constraint_converge(s_L=1500, s_a=500, cl_max_land=2.0, rho_ground=.002378, rho_sea=.002378)
...
x1 = 5
y1 = 5
plt.scatter(346.8, 485, color='red', s=70, zorder=2)
plt.text(346.8-2*x1, 485+3*y1, 'Ceres-100 ($S$ = 346.8 ft², $P_{shaft}$ = 485 hp, W = 9730 lbs)')
